...
buildah bud -t api-infoblox . | tee -a /dev/tty
...
{
    wallBanner="RPM automation-interface-api-infoblox-container post-install configuration message:\n"
    cd /usr/lib/api-infoblox
    ...
    diffOutput=$(podman exec api-infoblox diff /var/www/api_default_settings.py /var/www/api/api/settings.py | grep '^[<>].*' | grep -v SECRET | grep -v PASSWORD | grep -v VENV || true)
