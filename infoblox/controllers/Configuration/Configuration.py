...
from infoblox.serializers.Configuration.Configuration import ConfigurationSerializer as Serializer
...
class ConfigurationController(CustomController):
    ...
    def get(request: Request, configType: str) -> Response:
        ...
        try:
            ...
            lock = Lock("configuration", locals(), str(configType))
            if lock.isUnlocked():
                lock.lock()

                serializer = Serializer(
                    data=Configuration(configType=configType).repr()
                )
            ...
            Lock("configuration", locals(), str(configType)).release()
