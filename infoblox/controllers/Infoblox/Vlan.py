...
class InfobloxVlanController(CustomController):
    ...
    def get(request: Request, assetId: int, vlanId: int) -> Response:
        ...
        try:
            if CheckPermissionFacade.hasUserPermission(groups=user["groups"], action="vlan_get", assetId=assetId, isWorkflow=bool(workflowId)) or user["authDisabled"]:
                if workflowId and checkWorkflowPermission:
                    httpStatus = status.HTTP_204_NO_CONTENT
                else:
                    Log.actionLog("Vlan information", user)

                    lock = Lock("vlan", locals(), str(vlanId), workflowId=workflowId)
                    if lock.isUnlocked():
                        lock.lock()
