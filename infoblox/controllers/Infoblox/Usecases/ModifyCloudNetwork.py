...
class InfobloxModifyCloudNetworkController(CustomController):
    ...
    def put(request: Request, assetId: int, networkAddress: str) -> Response:
        ...
        try:
            if CheckPermissionFacade.hasUserPermission(groups=user["groups"], action="cloud_network_modify_put", assetId=assetId, isWorkflow=bool(workflowId)) or user["authDisabled"]:
