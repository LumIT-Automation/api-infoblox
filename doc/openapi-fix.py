...
parser.add_argument('-u','--urlFile',help='(infoblox url file)', required=True)
...
def getBlock(l: list, startIdx: int) -> list:
    ...
    try:
        for i in range(startIdx+1, len(l)):
            ...
            if re.match('\s+/infoblox/(.*/)?:', l[i]) or re.match('^[a-z]+:', l[i]):
                break
            else:
                b.append(l[i])
...
def getBlocksIndexes(l: list) -> (int, int):
    ...
    try:
        for i in range(len(l)):
            if re.match('\s+/infoblox/(.*/)?:', l[i]):  # line delimiting of a block or end last block.
                if not start:
                    start = i
                else:
                    blocks.append({
...
# Identify the urls parameters, using the InfobloxUrls.py file.
...
for url in urls:
    ...
    for idx in range(len(lines)):
        if re.match('\s+/infoblox/.*/:', lines[idx]): # get urls only from the inputFile.
            if re.match(urlMatch, lines[idx].strip().replace(':','')): # url line example in inputFile: /infoblox/<int:assetId>/network/<str:networkAddress>/:.
                urlData = dict()
                ...
                if not segments[-1]:  # empty segment.
                    segments.pop(-1)
                segments.insert(0,'infoblox') # urls in the urlFile doesn't have the infoblox prefix.
